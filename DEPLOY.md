...
A false positive from Cloudflare's new Workers + Static Assets validator. The Vite build already produces `404.html` for SPA fallback, so no `_redirects` file is needed. If you previously had `public/_redirects`, delete it and redeploy.

### "Invalid credentials" on first login
Migrations or seeds didn't run. In Render → **Settings → Start Command** set to `npm run db:setup && npm start`, redeploy, then change back to `npm start`.

### CORS errors in the browser
`CORS_ORIGIN` on Render must match the frontend URL exactly — no trailing slash, `https://` not `http://`.

### "Database connection failed" in Render logs
Use the **pooled** Neon URL (hostname contains `-pooler`). Make sure it ends with `?sslmode=require`.

### Verification email link is wrong
`FRONTEND_URL` must be set on Render to the Cloudflare URL. Email links use `${FRONTEND_URL}` for the welcome "Go to Login" button.

### Cold-start timeout on first request
30–60s is normal on Render's free tier. Set up the keep-alive (step 5), or upgrade to Render Starter ($7/mo) for no cold starts.

### `keep-alive.yml` failing in GitHub Actions
Check that the `BACKEND_HEALTH_URL` secret is set under **Settings → Secrets and variables → Actions**. The workflow logs a clear error if it's missing. A failing keep-alive does **not** affect live traffic — it only means the next user request will trigger a cold start.

## Free-tier limits

| Service | Limit | Sufficient for |
|---|---|---|
| Neon | 0.5 GB storage, no expiry | ~50 active users |
| Render | 512 MB RAM, 750 hrs/mo, 100 GB egress | Small/medium APIs |
| Cloudflare Pages | Unlimited bandwidth, 500 builds/mo | Any volume |

Upgrade paths: Render Starter ($7/mo) removes cold starts; Neon Launch ($19/mo) raises DB to 10 GB.
